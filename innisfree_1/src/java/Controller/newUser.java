...
public class newUser extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        Date date = new Date();
        Timestamp empCrDate = new Timestamp(date.getTime());
        String empCrBy = c.getEmpUsername();
        ...
        String emp_Status = jsonObject.getString("emp_Status");

        e.setPosition(position);
        e.setEmpCrDate(empCrDate);
        e.setEmpCrBy(empCrBy);
        e.setEmpUpDate(empUpDate);
        e.setEmpUpBy(empBUpy);
        e.setEmpUsername(username);
        e.setEmpName(empName);
        e.setEmpEmail(empemail);
        e.setEmpHp(empHp);
        e.setEmpPwd(password);
        e.setEmpStatus(emp_Status);

        DAEmployee dae = new DAEmployee();
        int row = dae.insertUser(e);

        JSONObject jsonResponse = new JSONObject();
        if (row > 0) {
            jsonResponse.put("status", "success");
            jsonResponse.put("message", "Name: < " + empName + " > Added Successfully!!!");

        } else {
            jsonResponse.put("status", "error");
            jsonResponse.put("message", "Name: < " + empName + " > Added UNSuccessfully!!!");
